...
카카오 소셜 로그인을 통해 사용자를 인증하고
JWT 토큰을 발급하는 기능을 구현했습니다.
...
| Type | 의미 | 사용 예시 |
|------|------|-----------|
| **Feat** | 새로운 기능 추가 | `Feat: 로그인 API 구현 (#12)` |
| **Fix** | 버그 수정 | `Fix: 회원 조회 null 응답 오류 수정 (#18)` |
| **Docs** | 문서 수정 | `Docs: README Git 컨벤션 수정 (#2)` |
| **Style** | 코드 스타일 변경 (로직 영향 없음) | `Style: import 정리 및 코드 포맷 수정 (#7)` |
| **Refactor** | 코드 리팩토링 (기능 변경 없음) | `Refactor: 인증 로직 분리 (#24)` |
| **Test** | 테스트 코드 추가/수정 | `Test: 회원가입 서비스 테스트 추가 (#30)` |
| **Chore** | 빌드/설정/기타 작업 | `Chore: Gradle Redis 의존성 추가 (#6)` |
| **Comment** | 주석 추가/수정 | `Comment: 인증 로직 설명 추가 (#25)` |
| **Rename** | 파일/폴더명 변경 | `Rename: MemberResponseDto 이름 변경 (#14)` |
| **Remove** | 파일/코드 삭제 | `Remove: 사용하지 않는 회원 DTO 삭제 (#21)` |
| **Init** | 프로젝트 초기 세팅 | `Init: Spring Boot 프로젝트 초기화 (#1)` |
| **Merge** | 브랜치 병합 | `Merge: feat/auth-3-kakao-login` |
| **!BREAKING CHANGE** | 하위 호환 불가 변경 | `!BREAKING CHANGE: 회원 API 응답 구조 변경 (#35)` |
